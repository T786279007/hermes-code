...
def codex_review_pr(
    ...
    """Perform code review on a pull request using Codex.

    Args:
        repo_path: Path to the repository
        pr_number: Pull request number
        model: Codex model to use for review
    ...
    Returns:
        Dictionary containing:
            ...
            - issues: List of ReviewIssue as dicts
            - summary: Review summary text
            - raw_output: Raw Codex output

    Raises:
        ValidationError: If parameters are invalid
        CLIError: If gh CLI or codex command fails
        ReviewParseError: If review output parsing fails
    """
    if not repo_path or not repo_path.strip():
        raise ValidationError("Repository path cannot be empty")
    ...
    logger.info(f"Starting Codex review for PR #{pr_number} in {repo_path}")
...
"""
        ...
        # Parse review results (same parser as Claude)
        issues, summary = _parse_review_output(codex_output)

        logger.info(f"Codex review complete: {len(issues)} issues found")
        ...
        logger.error(f"CLI error during Codex review: {e}")
        raise
    except ReviewParseError as e:
        logger.error(f"Parse error during Codex review: {e}")
        raise
